    ...
    for fname in files:
        ...
        for s in ['xS8', 'xS8aer']:
            if s in df.columns and N_level is not None:
                ...
                s8_amount = np.nansum(N_level * x) / 1e4
                break
...
def plot_short_range(df, tmin, tmax, show=True, save_path=None):
    ...
    # Compact figure: 10x3.5 inches
    fig, axes = plt.subplots(2, 1, figsize=(10, 3.5), sharex=True)

    # --- Surface Pressure ---
    axes[0].plot(df_short['time_rel_years'], df_short['surface_pres']/100000, label='Surface Pressure', color='black')
    axes[0].set_ylabel('Surface\nPressure (bar)', fontsize=13)

    # Optional: hide top subplot x-tick labels
    axes[0].tick_params(labelbottom=False)
    pressure_ticks = [0.6, 0.4, 0.2]  # 60000, 40000, 20000, 0
    axes[0].set_yticks(pressure_ticks)
    ...
    temp_ticks = [300, 250, 200, 150]  # 275, 225, 175, 125
    ax1.set_yticks(temp_ticks)

    l1 = ax1.plot(df_short['time_rel_years'], df_short['surface_temp'], color='blue', label='Surface Temp (K)')
    l2 = ax2.plot(df_short['time_rel_years'], df_short['S8'], color='red', linestyle='--', label='S8 Column')

    ax1.set_ylabel('Surface\nTemperature (K)', color='blue', fontsize=13, labelpad=10)
    ax2.set_ylabel('S$_8$ Column\n(molecules/cm²)', color='red', fontsize=13)
    ...
    plt.subplots_adjust(hspace=0)  # no gap between subplots
    plt.tight_layout()
